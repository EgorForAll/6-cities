...
const MainNoEmpty = ({offers}) => {
  ...
  const currentCity = currentPath.substring(11);
  const currentCityHotels = offers.filter(item => item.city.name.toLowerCase() === currentCity);
      ...
      <div className="cities">
        <div className="cities__places-container container">
          <section className="cities__places places">
            ...
            <b className="places__found">{currentCityHotels.length} places to stay in {capitalizeFirstLetter(currentCity)}</b>
            ...
            <div className="cities__places-list places__list tabs__content">
              {currentCityHotels.map((item) => item.city.name.toLowerCase() === currentCity ? <CardOffer key={item.id} offer={item}/> : null)}
            </div>
          </section>
